...
function extractWooProducts() {
  ...
  const selectors = [
    "lrv-product-grid",
    "[class*='lrv-product-grid'] [class*='product']",
    "[class*='price']",
  ];
  ...
  return Array.from(cards).map((card) => {
    ...
    const priceIns  = card.querySelector(".price .amount");
...
async function searchTwelve(keyword, pages = 1) {
  ...
  try {
    ...
    await navigateTo(page, `${BASE_URL}/product-category/${encodeURIComponent(keyword)}/`);
