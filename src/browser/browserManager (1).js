...
// Track in-progress launch so concurrent callers don't race to launch two browsers
let _launching = null;
...
async function getBrowser() {
  ...
  // If a launch is already in progress, wait for it rather than starting another
  if (_launching) return _launching;

  _launching = (async () => {
    try {
      // Close any lingering zombie browser before launching fresh
      if (_browser) {
      ...
      _browser = await puppeteer.launch({
        ...
        args: [
          ...
          // Required on Render free tier (512MB RAM)
          "--single-process",
          "--no-zygote",
        ],
      ...
      _launching = null; // always clear the lock
    }
  })();
